...
public class SimuName {
    ...
    public static void main(String[] args) {
        ...
        SimuName simuName = new SimuName();
        for (int i = 0; i < 60; i++) {

            // 姓氏随机生成
            String familyName = simuName.randFamilyName();
            // 名字依托于性别产生
            String randName = simuName.randName(simuName.randSex());
            String[] fixed = randName.split("-");

            String name = fixed[0];
            String sex = fixed[1];
            int age = simuName.randAge();
