...
public class CalciteSqlUtils {
  private static SqlParser.Config mysqlConfig =
  ...
  private static SqlParser.Config oralceConfig =
      SqlParser.configBuilder()
          .setLex(Lex.ORACLE)
          .setCaseSensitive(false) // 大小写敏感
          .setQuoting(Quoting.BACK_TICK)
  ...
  private static SqlParser.Config sqlserverConfig =
      SqlParser.configBuilder()
  ...
  public static void main(String[] args) {
    ...
    FileReader fileReader =
        new FileReader(
            "D:\\workspace\\zs\\code\\sqllineage\\src\\main\\resources\\sql\\query002.sql");
    String sql = fileReader.readString();

    try {
      List<Map<String, String>> list = handlerSqlTableAlias(DatabaseProduct.ORACLE, sql);
      // 打印
...
//      System.out.println(rt);
   ...
   */
  public static String handlerSqlParameterSubstitution(
      DatabaseProduct type, String sql, String param) throws SqlParseException {
    // 加载配置信息
   ...
   * @throws SqlParseException
   */
  public static List<Map<String, String>> handlerSqlTableAlias(DatabaseProduct type, String sql)
      throws SqlParseException {
    // 加载配置信息
    SqlParser sqlParser = null;
    switch (type) {
  ...
  private static String handlerSqlParameter(DatabaseProduct type, SqlNode node, String param) {
    ...
    // SqlDialect dialect = new SqlDialect(SqlDialect.DatabaseProduct.ORACLE,
    // SqlDialect.DatabaseProduct.ORACLE.name(),"");
    // 还原某个方言的sql
    // System.out.println(sqlSelect.toSqlString(OracleSqlDialect.DEFAULT).toString().replaceAll("\"",""));
    String sql = "";
    switch (type) {
      case ORACLE:
        SqlDialect.Context oracleSqlDialect =
        ...
        break;
      case MYSQL:
        SqlDialect.Context MYSQL_CONTEXT =
            SqlDialect.EMPTY_CONTEXT
        ...
        break;
      default:
        SqlDialect.Context DEFAULT_CONTEXT =
            SqlDialect.EMPTY_CONTEXT
  ...
  private static void handlerOperand(SqlBasicCall where, SqlSelect sqlSelect, String param) {
    ...
    for (int i = 0; i < operandList.size(); i++) {
      ...
      SqlNode paramName =
          operandStr.getOperandList().size() >= 2 ? operandStr : operandStr.getOperandList().get(0);
      ...
      if (checkKind(kind) && paramName.toString().contains(param)) {
        ...
        SqlCharStringLiteral literal =
            SqlCharStringLiteral.createCharString("jh", paramName.getParserPosition());
        operands[0] = sqlIdentifier;
        operands[1] = literal;
        SqlBasicCall operandCall =
            new SqlBasicCall(operator, operands, paramName.getParserPosition());
        // 匹配参数的值改成恒值如：bi=bi
...
class MySqlDialect extends SqlDialect {
  ...
  public void unparseDateTimeLiteral(
    ...
    if (literal instanceof SqlTimestampLiteral) {
      writer.literal(
          "TO_TIMESTAMP('" + literal.toFormattedString() + "', 'YYYY-MM-DD HH24:MI:SS.FF')");
...
//      SqlUtil.unparseFunctionSyntax(SqlLibraryOperators.SUBSTR_ORACLE, writer, call, false);
      ...
      switch (call.getKind()) {
        case FLOOR:
          ...
          SqlCall call2 =
              SqlFloorFunction.replaceTimeUnitOperand(
                  call, timeUnit.name(), timeUnitNode.getParserPosition());
