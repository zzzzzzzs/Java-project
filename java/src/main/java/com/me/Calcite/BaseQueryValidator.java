...
public class BaseQueryValidator {
  ...
  private static Map<String, String> extractWhereClauses(SqlNode node) {
    ...
    if (where != null) {
      ...
      if (where.operand(0).getKind().equals(SqlKind.IDENTIFIER)
        ...
        tableToPlaceHolder.put(where.operand(0).toString(), where.operand(1).toString());
        return tableToPlaceHolder;
      }
      ...
      while (!sqlBasicCallLeft.operand(0).getKind().equals(SqlKind.IDENTIFIER)
        ...
        tableToPlaceHolder.put(
            ((SqlBasicCall) sqlBasicCallLeft.operand(1)).operand(0).toString(),
      ...
      tableToPlaceHolder.put(
          sqlBasicCallLeft.operand(0).toString(), sqlBasicCallLeft.operand(1).toString());
      tableToPlaceHolder.put(
          sqlBasicCallRight.operand(0).toString(), sqlBasicCallRight.operand(1).toString());
      return tableToPlaceHolder;
    }
  ...
  public static void main(String[] args) throws SqlParseException {
    ...
    final List<String> tables = extractTableAliases(from);
    System.out.println(tables);
    // Extract where clauses, For above SQL query : [e.organization -> 'Tesla', s.organization ->
    // 'Tesla']
